  ...
  const uint8_t* restrict quadrature_permutation)
{
  ...
  static const double FE0_C0_D100_Q421[1][1][1][4] = {
    { { { -1.0, 1.0, 0.0, 0.0 } } }
  };
  static const double FE1_C1_D010_Q421[1][1][1][4] = {
    { { { -1.0, 0.0, 1.0, 0.0 } } }
  };
  static const double FE1_C2_D001_Q421[1][1][1][4] = {
    { { { -1.0, 0.0, 0.0, 1.0 } } }
  };
  // ------------------------
  // Section: Jacobian
  // Inputs: FE1_C2_D001_Q421, FE1_C1_D010_Q421, coordinate_dofs,
  // FE0_C0_D100_Q421 Outputs: J0_c5, J0_c4, J0_c7, J0_c8, J0_c1, J0_c6, J0_c3,
  // J0_c2, J0_c0
  double J0_c4 = 0.0;
  double J0_c8 = 0.0;
  double J0_c5 = 0.0;
  ...
  for (int iq = 0; iq < 1; ++iq) {
    ...
    // Inputs: fw0, fw3, fw6, FE1_C2_D001_Q421, fw2, FE1_C1_D010_Q421, fw1, fw5,
    // fw4, fw7, fw8, FE0_C0_D100_Q421 Outputs: A
    {
      double temp_0[4] = { 0 };
      for (int j = 0; j < 4; ++j) {
...
binary_search(int num_elements,
  ...
  const int32_t* __restrict__ colmap_sorted_indices)
{
  ...
  for (int i = thread_idx; i < num_active_cells; i += blockDim.x * gridDim.x) {
    ...
    for (int j = 0; j < 4; j++) {
      int vertex = vertex_indices_per_cell[c * 4 + j];
      for (int k = 0; k < 3; k++) {
        cell_vertex_coordinates[j * 3 + k] = vertex_coordinates[vertex * 3 + k];
      }
    }
      ...
      if (row < num_local_rows) {
        for (int k = 0; k < 4; k++) {
          ...
          if (column < num_local_columns) {
            int r;
            int err = binary_search(row_ptr[row + 1] - row_ptr[row],
                                    &column_indices[row_ptr[row]],
                                    column,
                                    &r);
            assert(
              !err &&
              "Failed to find column index in assemble_matrix_cell_global!");
            r += row_ptr[row];
            values[r] += Ae[j * 4 + k];
            // atomicAdd(&values[r],
            //   Ae[j*4+k]);
            ...
            int err = binary_search(
              num_local_offdiag_columns, colmap_sorted, column, &sorted_idx);
            assert(!err &&
                   "Failed to find offdiag column index in colmap_sorted!");
            int32_t colmap_idx = colmap_sorted_indices[sorted_idx];

            int r;
            err = binary_search(offdiag_row_ptr[row + 1] - offdiag_row_ptr[row],
  ...
  ufc_scalar_t* b)
{
  ...
  for (int c = thread_idx; c < num_cells; c += blockDim.x * gridDim.x) {
