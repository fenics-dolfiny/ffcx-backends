...
from ffcx_backends.cuda.optimizer import optimize
...
class IntegralGenerator:
    ...
    def generate_block_parts(
        ...
        for blockdata in blocklist:
            ...
            tables += table
            cached_arg_factors = []
            for i, factor in enumerate(arg_factors):
                # handle constant factor
                if not isinstance(factor, L.ArrayAccess):
                    cached_arg_factors.append(factor)
                    continue
                # the last index in the factor will come from B_indices
                key = (factor.array, factor.indices[:-1])
                if key not in temp_arg_factors:
                    nfactors = len(temp_arg_factors)
                    temp_arg_factors[key] = (
                        L.Symbol("temp_arg_factor_" + str(nfactors), dtype=L.DataType.SCALAR),
                        B_indices[i]
                    )
                cached_arg_factors.append(L.ArrayAccess(temp_arg_factors[key][0], [factor.indices[-1]]))
            # Define B_rhs = fw * arg_factors
            insert_rank = block_rank
            if self.ir.part == TensorPart.diagonal:
                insert_rank = 1
                B_indices = [B_indices[0]]
            B_rhs = L.float_product([fw] + cached_arg_factors)
        ...
        # define the cached arg_factor arrays
        cache_body = []
        cache_declarations = []
        cache_inputs = set()
        cache_outputs = []
        for key, value in temp_arg_factors.items():
            factor, index = value
            size = index.size()
            cache_declarations.append(L.ArrayDecl(factor, [size], [0]))
            table, indices = key
            cache_inputs.add(table)
            cache_outputs.append(factor)
            loop_body = L.Assign(
                L.ArrayAccess(factor, index), L.ArrayAccess(table, indices+(index,))
            )
            cache_body.append(L.ForRange(index, 0, size, [loop_body]))
